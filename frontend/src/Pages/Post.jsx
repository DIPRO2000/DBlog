...
//             `${import.meta.env.VITE_Backend_Url}/api/getpostbyid/${postId}`
//           );
...
//           // The backend returns an ARRAY, not {post: [...]}
//           setPosts(data.post);
...
//               <span>{format(new Date(Number(post.timestamp)*1000), 'MMMM d, yyyy')}</span>
//             </div>
//           </div>
...
export default function Post() {
  ...
  const formattedDate = post.timestamp
    ? format(new Date(Number(post.timestamp) * 1000), "MMMM d, yyyy")
    : "Unknown";

  // FIX TAGS HERE
  const parsedTags =
    typeof post.content?.tags === "string"
      ? JSON.parse(post.content.tags)
      : post.content?.tags || [];
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                {post.author_wallet && (
                  <p className="text-xs text-slate-500 flex items-center gap-1">
                    <Wallet className="w-3 h-3" />
                    {post.author_wallet.slice(0, 6)}...
                    {post.author_wallet.slice(-4)}
                  </p>
                )}
              </div>
            </div>
